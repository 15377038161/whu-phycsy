...
- 创建班级时设置“最低通关实验数”，默认 3。
- 学生首页先选择大关；进入实验后按顺序完成页面式小关卡，允许返回已完成关复查。
- 教师在“实验小关卡与数据配置”中维护结构化配置，支持数值、短文本、单选、多选、表格、图片及拟合关；拟合关必须明确绑定来源步骤、字段和横纵坐标列。
- “实验原理、视频与学习资料”支持演示视频、PDF、PPT/PPTX；PDF 可直接预览，PPT/PPTX 应同时上传同名 PDF 预览版。
- 本交付包已内置五份首批讲义，位于 static/materials/；教师后续上传资料会随新发布版本冻结。
- 正式报告提交前服务器会检查全部必做步骤；操作数据、图片、在线拟合或 Excel 兜底结果会同步进报告草稿，检测到学生已修改系统块时不会静默覆盖。
