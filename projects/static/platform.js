...
let activeLayer=null,lastLayerTrigger=null;
const layerFocusable='a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])';
function visibleLayer(){return document.querySelector("[data-layer]:not(.is-hidden),.modal-backdrop:not(.is-hidden)")}
function openLayer(name,trigger=null){
  const layer=document.querySelector(`[data-layer="${name}"],[data-modal="${name}"]`);if(!layer)return;
  if(activeLayer&&activeLayer!==layer)closeLayer(activeLayer,false);
  if(layer.parentElement!==document.body)document.body.appendChild(layer);
  lastLayerTrigger=trigger||document.activeElement;activeLayer=layer;if(trigger)trigger.setAttribute("aria-expanded","true");layer.classList.remove("is-hidden");layer.setAttribute("aria-hidden","false");document.body.classList.add("layer-open");refreshIcons();
  requestAnimationFrame(()=>{const panel=layer.querySelector('[role="dialog"]')||layer;(panel.querySelector("[autofocus]")||panel.querySelector(layerFocusable))?.focus()});
}
function closeLayer(layer=activeLayer,restoreFocus=true){
  if(!layer)return;layer.classList.add("is-hidden");layer.setAttribute("aria-hidden","true");if(layer===activeLayer)activeLayer=null;if(!visibleLayer())document.body.classList.remove("layer-open");lastLayerTrigger?.setAttribute?.("aria-expanded","false");if(restoreFocus&&lastLayerTrigger instanceof HTMLElement)lastLayerTrigger.focus();
}
function openModal(name,trigger=null){openLayer(name,trigger)}
function closeModal(modal){closeLayer(modal)}
function showToast(message,type="success"){
  let stack=$("[data-toast-stack]");if(!stack){stack=document.createElement("div");stack.className="toast-stack";stack.dataset.toastStack="";stack.setAttribute("aria-live","polite");document.body.appendChild(stack)}
  const toast=document.createElement("div");toast.className=`toast ${type}`;toast.innerHTML=`<i data-lucide="${type==="error"?"circle-alert":"circle-check"}"></i><span>${escapeHtml(message)}</span><button type="button" aria-label="关闭通知" data-dismiss-toast><i data-lucide="x"></i></button>`;stack.appendChild(toast);refreshIcons();setTimeout(()=>toast.remove(),5000)
}

function setupGlobalUX(){
  const back=$('[data-smart-back]'),network=$('[data-network-status]');
  back?.addEventListener("click",()=>{let sameOrigin=false;try{sameOrigin=Boolean(document.referrer)&&new URL(document.referrer).origin===location.origin}catch{}if(sameOrigin&&history.length>1)history.back();else location.assign(back.dataset.fallbackUrl||"/")});
  $$('.primary-nav a.active,.teacher-subnav a.active').forEach(link=>link.setAttribute("aria-current","page"));
  const syncNetwork=()=>{const offline=!navigator.onLine;document.body.classList.toggle("is-offline",offline);network?.classList.toggle("is-hidden",!offline);$$('form button[type="submit"],form button:not([type]),[data-requires-online]').forEach(control=>{if(offline&&!control.disabled){control.dataset.offlineDisabled="true";control.disabled=true;control.setAttribute("aria-disabled","true")}else if(!offline&&control.dataset.offlineDisabled){control.disabled=false;control.removeAttribute("aria-disabled");delete control.dataset.offlineDisabled}})};
  window.addEventListener("online",()=>{syncNetwork();showToast("网络已恢复，可以继续保存或提交。")});
  window.addEventListener("offline",()=>{syncNetwork();showToast("网络连接已断开，尚未提交的内容请暂时保留在当前页面。","error")});
  syncNetwork();
  let lastInvalid=0;
  document.addEventListener("invalid",event=>{const field=event.target;if(!(field instanceof HTMLElement))return;field.classList.add("field-invalid");field.setAttribute("aria-invalid","true");const now=Date.now();if(now-lastInvalid>800){showToast(field.validationMessage||"请检查必填项和输入格式。","error");lastInvalid=now}requestAnimationFrame(()=>field.focus())},true);
  document.addEventListener("input",event=>{const field=event.target;if(field instanceof HTMLInputElement||field instanceof HTMLTextAreaElement||field instanceof HTMLSelectElement){if(field.validity.valid){field.classList.remove("field-invalid");field.removeAttribute("aria-invalid")}}});
  document.addEventListener("submit",event=>{const form=event.target;if(!(form instanceof HTMLFormElement)||event.defaultPrevented||form.matches('[data-student-ai-form],[data-teacher-ai-form]'))return;const submitter=event.submitter;if(!(submitter instanceof HTMLElement))return;submitter.setAttribute("aria-busy","true");submitter.classList.add("is-loading");setTimeout(()=>{if(document.contains(submitter))submitter.setAttribute("disabled","")},0)});
...
function setupLevelPicker(){
  const form=$("[data-level-picker-form]");if(!form)return;
  const required=Number(form.dataset.required||0),count=$("[data-level-selected-count]",form),submit=$("[data-level-picker-submit]",form),inputs=$$("input[name='selected_versions']",form);
  const sync=()=>{const selected=inputs.filter(input=>input.checked).length;if(count)count.textContent=String(selected);if(submit){submit.disabled=selected<required;submit.setAttribute("aria-disabled",String(selected<required))}};
  inputs.forEach(input=>input.addEventListener("change",sync));sync();
}
...
function setupQuiz(){
  ...
  $("[data-quiz-review-toggle]")?.addEventListener("click",event=>{const nextHidden=!($(".quiz-answer-card",form)?.hidden);$$('.quiz-answer-card',form).forEach(card=>{card.hidden=nextHidden});event.currentTarget.textContent=nextHidden?"查看逐题解析":"收起逐题解析"});
  $("[data-quiz-skip]")?.addEventListener("click",()=>activateTab("challenge"));
}

function setupChallenge(){
  const root=$("[data-challenge-workspace]");if(!root)return;
  const cards=$$("[data-challenge-step]",root),targets=$$("[data-challenge-target]",root),counter=$("[data-challenge-current]",root);let active=Number(root.dataset.currentStep||0);
  const collect=card=>{const data={};$$("[data-step-field]",card).forEach(field=>data[field.dataset.stepField]=field.value.trim());$$("[data-choice-field]",card).forEach(group=>{const checked=$$("input:checked",group).map(input=>input.value);data[group.dataset.choiceField]=group.dataset.choiceType==="single_choice"?(checked[0]||""):checked});$$("[data-table-field]",card).forEach(group=>{data[group.dataset.tableField]=$$("tr",$("[data-step-table-rows]",group)).map(row=>Object.fromEntries($$("[data-table-column]",row).map(input=>[input.dataset.tableColumn,input.value.trim()]))).filter(row=>Object.values(row).some(Boolean))});return data};
  const syncFitPreview=card=>{const fit=$("[data-fit-workspace]",card);if(!fit)return;const source=cards.find(item=>item.dataset.stepId===fit.dataset.sourceStepId),field=source?.querySelector(`[data-table-field="${CSS.escape(fit.dataset.sourceFieldKey||"")}"]`),target=$("[data-fit-rows]",fit);if(!field||!target)return;const keys=[fit.dataset.sourceX,fit.dataset.sourceY].filter(Boolean),sourceRows=$$("tr",$("[data-step-table-rows]",field)).map(row=>keys.map(key=>$(`[data-table-column="${CSS.escape(key)}"]`,row)?.value.trim()||"")).filter(row=>row.some(Boolean));target.innerHTML=sourceRows.map((row,index)=>`<tr><td>${String(index+1).padStart(2,"0")}</td>${row.map((value,column)=>`<td><input readonly data-fit-col="${column}" value="${escapeHtml(value)}"></td>`).join("")}</tr>`).join("");target.dispatchEvent(new Event("input",{bubbles:true}))};
  const activate=index=>{active=Math.max(0,Math.min(index,cards.length-1));cards.forEach(card=>card.classList.toggle("active",Number(card.dataset.stepIndex)===active));targets.forEach(button=>button.classList.toggle("active",Number(button.dataset.challengeTarget)===active));if(counter)counter.textContent=String(active+1);root.dataset.currentStep=String(active);syncFitPreview(cards[active]);refreshIcons();root.scrollIntoView({behavior:matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth",block:"start"})};
  const updateTimeline=result=>{const completed=new Set(result.completed_steps||[]);targets.forEach(button=>{const index=Number(button.dataset.challengeTarget),done=completed.has(index),allowed=done||index<=Number(result.current_step);button.classList.toggle("done",done);button.disabled=!allowed;const small=$("small",button);if(small)small.textContent=done?"已完成":index===Number(result.current_step)?"当前关卡":"待完成"})};
  const save=async(card,forceSync=false)=>{const state=$("[data-step-state]",card),errors=$("[data-step-errors]",card),button=$("[data-challenge-save]",card);state.textContent="正在校验并保存…";errors.textContent="";button.disabled=true;try{const response=await fetch(card.dataset.stepUrl,{method:"PUT",headers:csrfHeaders({"Content-Type":"application/json"}),body:JSON.stringify({data:collect(card),completed:true,force_sync:forceSync})});const result=await response.json();if(!response.ok){if(result.validation_errors)errors.innerHTML=result.validation_errors.map(item=>`<p>${escapeHtml(item.message)}</p>`).join("");throw new Error(result.error||"小关卡保存失败")}if(result.report_sync_conflict&&!forceSync){const refresh=confirm("报告中对应的系统生成内容已被你修改。是否用最新实验数据刷新该部分？\n选择“取消”将保留报告中的手工修改。");if(refresh)return save(card,true);showToast("已保留报告中的手工修改；实验进度仍已保存。")}updateTimeline(result);state.textContent=result.level_completed?"全部小关卡已完成":"本关已保存并同步到报告";document.dispatchEvent(new CustomEvent("report:server-sync"));if(result.level_completed){showToast("恭喜闯关成功！实验数据与拟合结果已同步到报告。","success");activate(active)}else activate(Number(result.current_step));return true}catch(error){state.textContent=error.message;showToast(error.message,"error");return false}finally{button.disabled=false}};
  root.addEventListener("click",event=>{const target=event.target.closest("[data-challenge-target]");if(target&&!target.disabled){activate(Number(target.dataset.challengeTarget));return}const card=event.target.closest("[data-challenge-step]");if(!card)return;if(event.target.closest("[data-challenge-prev]")){activate(Number(card.dataset.stepIndex)-1);return}if(event.target.closest("[data-challenge-save]"))save(card);if(event.target.closest("[data-add-step-row]")){const body=$("[data-step-table-rows]",event.target.closest("[data-table-field]")),source=$("tr",body);if(!source)return;const row=source.cloneNode(true);$$("input",row).forEach(input=>input.value="");$("td",row).textContent=String(body.children.length+1).padStart(2,"0");body.appendChild(row)}});
  root.addEventListener("change",async event=>{if(!event.target.matches("[data-step-image]"))return;const card=event.target.closest("[data-challenge-step]"),file=event.target.files?.[0];if(!file)return;const state=$("[data-step-state]",card),body=new FormData();body.append("image",file);state.textContent="正在上传图片…";try{const response=await fetch(card.dataset.stepImageUrl,{method:"POST",headers:csrfHeaders(),body}),result=await response.json();if(!response.ok)throw new Error(result.error||"图片上传失败");const image=document.createElement("img");image.src=result.url;image.alt="本步骤图片";$("[data-step-images]",card)?.appendChild(image);state.textContent="图片已上传并同步到报告";document.dispatchEvent(new CustomEvent("report:server-sync"))}catch(error){state.textContent=error.message;showToast(error.message,"error")}finally{event.target.value=""}});
  activate(active);
}
...
function setupFit(){
  ...
  const template=root.dataset.fitTemplate||"linear",experiment=root.dataset.exp||root.closest("[data-challenge-workspace]")?.dataset.exp||"";const defaults=presetFit(experiment,template);code.value=defaults;if(editor)editor.value=defaults;
  ...
  const persistFit=async result=>{if(!root.dataset.fitSaveUrl)return result;const response=await fetch(root.dataset.fitSaveUrl,{method:"POST",headers:csrfHeaders({"Content-Type":"application/json"}),body:JSON.stringify({result})}),saved=await response.json();if(!response.ok)throw new Error(saved.error||"拟合结果保存失败");document.dispatchEvent(new CustomEvent("report:server-sync"));return saved.result};
  async function runFit({silentInvalid=false}={}){if(runButton.disabled)return;clearTimeout(autoTimer);const rowData=completePairs();if(!rowData){if(!silentInvalid)state.textContent="请先在前序小关填写至少两行完整的数值数据";return}runButton.disabled=true;runButton.setAttribute("aria-busy","true");try{const cols=getColumns(),x=rowData.map(row=>Number(row[0])),y=rowData.map(row=>Number(row[1])),extra={};for(let i=2;i<cols.length;i++)extra[`col_${cols[i]}`]=rowData.map(row=>row[i]?Number(row[i]):null).filter(value=>value!==null);pyodide=await ensureRuntime();state.textContent="正在检查代码所需模块…";await withTimeout(pyodide.loadPackagesFromImports(code.value),120000,"代码依赖模块加载超时");state.textContent="模块已就绪 · 正在执行拟合并生成图表…";pyodide.globals.set("x",pyodide.toPy(x));pyodide.globals.set("y",pyodide.toPy(y));for(const[key,value]of Object.entries(extra))if(value.length)pyodide.globals.set(key,pyodide.toPy(value));await withTimeout(pyodide.runPythonAsync(code.value),60000,"拟合执行超时，请检查数据或预置代码");const json=await pyodide.runPythonAsync(`import json, io, base64\n_buf=io.BytesIO()\nplt.gcf().savefig(_buf,format='png',dpi=140,bbox_inches='tight')\njson.dumps({'final_parameters':result.get('final_parameters',{}),'metrics':result.get('metrics',{}),'plot_data_url':'data:image/png;base64,'+base64.b64encode(_buf.getvalue()).decode()})`),fit=JSON.parse(json);fit.code=code.value;fit.formula=formula.value;fit.initial_parameters=JSON.parse(initial.value||"{}");const check=await fetch(endpoint("fittingValidateUrl","/api/fitting/validate"),{method:"POST",headers:csrfHeaders({"Content-Type":"application/json"}),body:JSON.stringify(fit)}),checked=await check.json();if(!check.ok)throw new Error(checked.error||"结果校验失败");const savedFit=await persistFit(checked.result),value=JSON.stringify(savedFit);fitHidden.value=value;if(submitHidden)submitHidden.value=value;output.hidden=false;placeholder.hidden=true;$("img",output).src=savedFit.plot_data_url;$("pre",output).textContent=JSON.stringify({final_parameters:savedFit.final_parameters,metrics:savedFit.metrics},null,2);state.textContent="拟合完成 · 结果已保存并同步到报告";$(".fit-result-card .status",root).textContent="拟合成功";$(".fit-result-card .status",root).className="status published"}catch(error){const missing=String(error.message).match(/(?:No module named|ModuleNotFoundError:?)[ '\"]+([^'\"\\s]+)/i);state.textContent=missing?`无法自动运行：浏览器环境未提供模块 ${missing[1]}，请使用 Excel 兜底或联系教师调整代码`:`运行失败：${error.message}`}finally{runButton.disabled=false;runButton.removeAttribute("aria-busy")}}
  runButton?.addEventListener("click",()=>runFit());
  $("[data-save-fit-fallback]",root)?.addEventListener("click",async event=>{const button=event.currentTarget,chart=$("[data-fit-fallback-chart]",root)?.files?.[0],parameters=$("[data-fit-fallback-parameters]",root)?.value.trim()||"{}";if(!chart){state.textContent="请先选择 Excel 导出的拟合结果图";return}button.disabled=true;try{JSON.parse(parameters);const body=new FormData();body.append("parameters",parameters);body.append("chart",chart);const response=await fetch(root.dataset.fitFallbackUrl,{method:"POST",headers:csrfHeaders(),body}),saved=await response.json();if(!response.ok)throw new Error(saved.error||"Excel 拟合结果保存失败");fitHidden.value=JSON.stringify(saved.result);state.textContent="Excel 拟合结果已保存并同步到报告";$(".fit-result-card .status",root).textContent="Excel 结果已保存";$(".fit-result-card .status",root).className="status published";if(saved.chart_url){output.hidden=false;placeholder.hidden=true;$("img",output).src=saved.chart_url;$("pre",output).textContent=JSON.stringify(saved.result.final_parameters,null,2)}document.dispatchEvent(new CustomEvent("report:server-sync"));showToast("Excel 拟合结果已保存","success")}catch(error){state.textContent=`保存失败：${error.message}`}finally{button.disabled=false}});
  ...
  document.addEventListener("workspace:tab",event=>{if(event.detail.name!=="challenge")return;ensureRuntime().catch(error=>state.textContent=`计算环境准备失败：${error.message}`);scheduleAutoRun()});
  rebuildHeader();sync();
}
...
function setupQuestionManager(){
  ...
  $("[data-question-delete]",form).addEventListener("click",()=>{const item=activeQuestion();if(!item)return;showActionConfirm("确认删除当前题目？保存草稿后删除才会生效。",()=>{const index=questions.indexOf(item);questions.splice(index,1);activeId=questions[index]?.id||questions[index-1]?.id||null;renderList();renderEditor()})});
...
function setupReportEditor(){
  ...
  document.addEventListener("report:server-sync",async()=>{if(locked||dirty||!root.dataset.getUrl){if(dirty)setState("实验数据已更新 · 保存当前编辑后刷新报告","editing");return}try{const response=await fetch(root.dataset.getUrl),draft=await response.json();if(!response.ok)throw new Error(draft.error||"报告刷新失败");model=draft.content;lockVersion=draft.lock_version;root.dataset.lockVersion=String(lockVersion);renderEditors();renderPaper();setState("实验数据已同步到报告","saved")}catch(error){setState(error.message,"error")}});
...
function setupStudentAssistant(){
  const panel=$("[data-student-ai]"),launcher=$("[data-open-student-ai]"),backdrop=$("[data-student-ai-backdrop]"),close=$("[data-close-student-ai]"),form=$("[data-student-ai-form]"),question=$("[data-ai-question]"),messages=$("[data-ai-messages]");if(!panel||!launcher||!form)return;
  const history=[];
  const setOpen=open=>{panel.classList.toggle("open",open);backdrop?.classList.toggle("open",open);panel.setAttribute("aria-hidden",String(!open));launcher.setAttribute("aria-expanded",String(open));launcher.classList.toggle("is-hidden",open);document.body.classList.toggle("layer-open",open);if(open)setTimeout(()=>question.focus(),180);else launcher.focus()};
  ...
  launcher.addEventListener("click",()=>setOpen(true));close.addEventListener("click",()=>setOpen(false));backdrop?.addEventListener("click",()=>setOpen(false));form.addEventListener("submit",event=>{event.preventDefault();if(!form.classList.contains("busy"))ask(question.value)});question.addEventListener("keydown",event=>{if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();form.requestSubmit()}});$$("[data-ai-prompt]",panel).forEach(button=>button.addEventListener("click",()=>ask(button.dataset.aiPrompt)));document.addEventListener("keydown",event=>{if(event.key==="Escape"&&panel.classList.contains("open"))setOpen(false)});
}
...
function setupAutoOpenModal(){
  const modal=$("[data-auto-open-modal]");if(!modal)return;
  window.setTimeout(()=>openModal(modal.dataset.autoOpenModal),180);
}
...
function setupTeacherAssistant(){
  const panel=$("[data-teacher-ai]"),launcher=$("[data-teacher-ai-launcher]"),backdrop=$("[data-teacher-ai-backdrop]"),form=$("[data-teacher-ai-form]"),question=$("[data-teacher-ai-question]"),messages=$("[data-teacher-ai-messages]"),course=$("[data-teacher-ai-course]"),sync=$("[data-teacher-ai-sync]");
  if(!panel||!form||!question||!messages)return;
  let loaded=false,busy=false;
  const setOpen=open=>{panel.classList.toggle("open",open);backdrop?.classList.toggle("open",open);panel.setAttribute("aria-hidden",String(!open));launcher?.setAttribute("aria-expanded",String(open));launcher?.classList.toggle("is-hidden",open);document.body.classList.toggle("layer-open",open);if(open){loadCourses();setTimeout(()=>question.focus(),120)}else launcher?.focus()};
...
function showConfirm(form,message,submitter){
  showActionConfirm(message,()=>{form.dataset.confirmed="true";form.requestSubmit(submitter)})
}
function showActionConfirm(message,onAccept){
  const modal=document.createElement("div");modal.className="modal-backdrop";modal.dataset.layer="action-confirm";modal.innerHTML=`<section class="dialog-card confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="actionConfirmTitle"><button class="modal-close" data-close-layer aria-label="关闭"><i data-lucide="x"></i></button><span class="eyebrow">CONFIRM ACTION</span><h2 id="actionConfirmTitle">确认当前操作</h2><p>${escapeHtml(message)}</p><div class="button-row"><button class="secondary-button" type="button" data-close-layer>取消</button><button class="danger-button" type="button" data-accept-confirm>确认继续</button></div></section>`;document.body.appendChild(modal);activeLayer=modal;lastLayerTrigger=document.activeElement;document.body.classList.add("layer-open");refreshIcons();modal.querySelector("[data-close-layer]")?.focus();modal.addEventListener("click",event=>{if(event.target===modal||event.target.closest("[data-close-layer]")){closeLayer(modal);modal.remove()}if(event.target.closest("[data-accept-confirm]")){closeLayer(modal,false);modal.remove();onAccept()}})
}
...
document.addEventListener("click",event=>{
  ...
  const open=event.target.closest("[data-open-modal],[data-open-layer]");if(open){event.preventDefault();openLayer(open.dataset.openLayer||open.dataset.openModal,open)}
  const close=event.target.closest("[data-close-modal],[data-close-layer]");if(close){const href=close.getAttribute("href");event.preventDefault();closeLayer(close.closest("[data-layer],.modal-backdrop"));if(href?.startsWith("#"))requestAnimationFrame(()=>document.querySelector(href)?.scrollIntoView({behavior:"smooth",block:"start"}))}
  const dismiss=event.target.closest("[data-dismiss-toast]");if(dismiss){event.preventDefault();dismiss.closest(".toast")?.remove()}
  const layer=event.target.closest("[data-layer],.modal-backdrop");if(layer&&event.target===layer&&layer.dataset.backdropClose!=="false")closeLayer(layer);
...
document.addEventListener("keydown",event=>{const layer=visibleLayer();if(!layer)return;if(event.key==="Escape"){event.preventDefault();closeLayer(layer);return}if(event.key!=="Tab")return;const panel=layer.querySelector('[role="dialog"]')||layer,items=$$(layerFocusable,panel).filter(item=>item.offsetParent!==null);if(!items.length)return;const first=items[0],last=items.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}});

document.addEventListener("DOMContentLoaded",()=>{
  setupGlobalUX();setupAmbient();setupExperimentGallery();setupAchievements();setupAutoOpenModal();setupWorkspaceStepNavigation();setupStudentRecords();setupLevelPicker();setupQuiz();setupChallenge();setupFit();setupFilters();setupExperimentDraftAssistant();setupEditor();setupQuestionManager();setupReportEditor();setupReport();setupPrincipleFormula();setupAcademicFormulas();setupStudentAssistant();setupTeacherAssistant();setupTeacherAnalytics();refreshIcons();
  $$('.flash').forEach(item=>{showToast(item.textContent.trim(),item.classList.contains("error")?"error":"success");item.remove()});
});
